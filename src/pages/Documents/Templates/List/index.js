...
import DocTemplatesColumns, { selectRow } from "./Columns"
import DocTemplatesConfirmModal from "./ConfirmModal"

// Components
import NotFound from "pages/Utility/pages-404-content"

const DocTemplates = props => {
  if (
  ...
  const [customerList, setDocTemplatesList] = useState([])
  const pageOptions = {
    sizePerPage: 20,
    totalSize: totalDocTemplates,
    custom: true,
  ...
  useEffect(() => {
    getDocTemplatesSuccess({ data: [], totalcount: 0 })
    onGetDocTemplates(filter)
    setDocTemplatesList(documentTemplates)
  }, [onGetDocTemplates, JSON.stringify(filter)])
  ...
  const removeConfirm = () => {
    ...
    onDeleteDocTemplate({ _id: customerId }, onComplete)
  }
  ...
  const handleDocTemplateStatusChange = status => {
    const callback = () => {
      setSelected([])
      onGetDocTemplates(filter)
    }

    onPutDocTemplatesStatus({ _id: selected, status }, callback)
  }

  return (
    <React.Fragment>
      <DocTemplatesConfirmModal
      ...
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs
            title="DocTemplates"
            breadcrumbItem={props.t("Document Templates")}
            breadcrumbItems={[
              {
                                ...
                                <div className="text-sm-left text-md-right">
                                  {accesses.canAdd && (
                                    <Button
                                      ...
                                      onClick={() =>
                                        history.push("/document-templates/add")
                                      }
                                    >
                                      <i className="mdi mdi-plus mr-1" />
                                      {props.t("add_new_document_templates")}
                                    </Button>
                                  )}
                                </div>
                              </Col>
...
DocTemplates.propTypes = {
  ...
  totalDocTemplates: PropTypes.number,
  getDocTemplatesSuccess: PropTypes.func,
  onGetDocTemplates: PropTypes.func,
  onDeleteDocTemplate: PropTypes.func,
  onPutDocTemplatesStatus: PropTypes.func,
}

const mapStateToProps = ({ Settings, documentTemplates }) => ({
  settings: Settings.settings,
  loading: documentTemplates.loading,
  documentTemplates: documentTemplates.documentTemplates,
  totalDocTemplates: documentTemplates.totalDocTemplates,
})

const mapDispatchToProps = dispatch => ({
  getDocTemplatesSuccess: data => dispatch(getDocTemplatesSuccess(data)),
  onGetDocTemplates: data => dispatch(getDocTemplates(data)),
  onDeleteDocTemplate: (data, callback) =>
    dispatch(deleteDocTemplate(data, callback)),
  onPutDocTemplatesStatus: (data, callback) =>
    dispatch(putDocTemplatesStatus(data, callback)),
})
