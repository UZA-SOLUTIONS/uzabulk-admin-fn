...
const CustomersColumns = (history, toggleConfirmModal, accesses, t) => [
  {
    dataField: "name",
    text: t("name"),
    sort: true,
  ...
  },
  {
    ...
    formatter: (_, row) => (
      <>
        <Link
          to={`/document-templates/${row._id}/fields?template_name=${row?.name}`}
        ...
        <Link
          to={`/document-templates/${row._id}/edit`}
          className="mr-3 text-secondary"
        >
        ...
        {accesses?.canDelete && !["qualificationInfo", "vehicleInfo"].includes(row.type) && (
          <Link
            to="#"
            onClick={e => {
              e.preventDefault()
