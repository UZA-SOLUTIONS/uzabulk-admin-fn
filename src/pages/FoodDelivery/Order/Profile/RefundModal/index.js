...
const RefundModal = props => {
  ...
  useEffect(() => {
    return () => {
      setfields({ admin: 0, vendor: 0, driver: 0, reason: "" })
    }
  }, [isOpen])
...
<Col md={6}>
              <FormGroup>
                <Label>
                  {props.t(GET_SERVICE_NAME({ storeType }))} {props.t("amount")}
                </Label>
            ...
            <Col md={6}>
              <FormGroup>
              <Label>{store !=="SERVICEPROVIDER" ? props.t("driver_amount") : props.t("serviceprovider_amount")}</Label>
