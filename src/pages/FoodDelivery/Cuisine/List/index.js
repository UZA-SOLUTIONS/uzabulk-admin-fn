...
import { EXPORT_FD_CUISINES } from "helpers/url_helper"
                            ...
                            <Row>
                              ...
                              >
                                <div className="text-sm-left text-md-right">
                                  <ExportCSVButton url={EXPORT_FD_CUISINES} />
                                      ...
                                      onClick={() =>
                                        history.push(
                                          `${["premium"].includes(
                                            ...
                                            : "/" +
                                            activeStoreType?.storeType?.toLowerCase()
                                          }/${handleAddpagename()}/add${!!_vendorIdParam
                                            ? "/" + vendorIdParam
                                            : ""
                                          }`
                                        )
...
FdCuisines.propTypes = {
  ...
  onDeleteFdCuisine: PropTypes.func,
  onPutFdCuisinesStatus: PropTypes.func,
}

const mapStateToProps = ({ Login, Billing, Settings, FD_Cuisines }) => ({
  role: Login?.user?.role || "",
  vendorId: Login?.user?._id || "",
  loading: FD_Cuisines.loading,
  fdCuisines: FD_Cuisines.fdCuisines,
  totalFdCuisines: FD_Cuisines.totalFdCuisines,
...
const mapDispatchToProps = dispatch => ({
  ...
  onDeleteFdCuisine: (storeType, data, callback) =>
    dispatch(deleteFdCuisine(storeType, data, callback)),
  onPutFdCuisinesStatus: (storeType, data, callback) =>
    dispatch(putFdCuisinesStatus(storeType, data, callback)),
  onExportFdCuisines: storeType => dispatch(exportFdCuisines(storeType)),
})
