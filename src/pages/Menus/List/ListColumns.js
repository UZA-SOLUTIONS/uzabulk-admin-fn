...
const ListColumns = (history, toggleConfirmModal, accesses, t) => [
  {
    ...
    formatter: (_, row) =>
      row.type !== "CONTACT_US" ? (
        <Link
          to={`/${"menus"}/${row._id
            }/sections?content_name=${row?.label}`}
          className="p-0 waves-effect waves-light btn-sm font-weight-bold font-size-13"
        >
          <span id={`label-${row._id}-tooltip`}>{row?.label}</span>
          <UncontrolledTooltip
            placement="top"
            target={`label-${row._id}-tooltip`}
          >
            {t("view_details")}
          </UncontrolledTooltip>
        </Link>
      ) : (
        <span className="font-weight-bold font-size-13">{row.label}</span>
      ),
  },
  {
    dataField: "status",
