...
import { getCampaigns, deleteCampaign, putCampaignsStatus } from "store/actions"
...
const Campaigns = ({
  ...
  useEffect(() => {
    onGetCampaigns(filter)
    setCampaignsList(campaigns)
  }, [onGetCampaigns, JSON.stringify(filter)])
  ...
  const toggleConfirmModal = campaignId => {
    setConfirmModal(prevState => ({ isOpen: !prevState.isOpen, campaignId }))
  }
  ...
  const removeConfirm = () => {
    const { campaignId, isOpen } = confirmModal

    const onComplete = () => {
      onGetCampaigns(filter)
    }

    setConfirmModal({ isOpen: !isOpen, campaignId: null })
    onDeleteCampaign({ _id: campaignId }, onComplete)
  }
  ...
  const removeSelectedConfirm = () => {
    ...
    onPutCampaignsStatus({ _id: selected, status: "archived" }, onComplete)
  }
  ...
  const handleCampaignStatusChange = status => {
    ...
    onPutCampaignsStatus({ _id: selected, status }, callback)
  }
    ...
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs
            title="Campaigns"
            breadcrumbItem={props.t("campaigns")}
            breadcrumbItems={[{ title: props.t("campaigns") }]}
          />
...
Campaigns.propTypes = {
  ...
  onPutCampaignsStatus: PropTypes.func,
}

const mapStateToProps = ({ Settings, Campaigns }) => ({
...
const mapDispatchToProps = dispatch => ({
  onGetCampaigns: data => dispatch(getCampaigns(data)),
  onDeleteCampaign: (data, callback) =>
    dispatch(deleteCampaign(data, callback)),
  onPutCampaignsStatus: (data, callback) =>
    dispatch(putCampaignsStatus(data, callback)),
})

export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(withTranslation()(Campaigns))
)
