...
const CampaignTemplate = ({
  ...
  history,
  campaignTemplates,
  totalCampaignTemplate,
  onGetCampaignTemplates,
  onDeleteCampaignTemplate,
  onPutCampaignTemplatesStatus,
  loading,
  ...props
}) => {
  if (
    !props.settings?.hideThings?.find(element => element.type === "isMarketing")
      ?.value
  ) {
    return <NotFound />
  }
  ...
  const [campaignTemplatesList, setCampaignTemplateList] = useState([])
  const pageOptions = {
    sizePerPage: 20,
    totalSize: totalCampaignTemplate,
    custom: true,
  ...
  useEffect(() => {
    ...
    setCampaignTemplateList(campaignTemplates)
  }, [onGetCampaignTemplates, JSON.stringify(filter)])
  ...
  const toggleConfirmModal = campaignTemplateId => {
  ...
  const removeConfirm = () => {
    const { campaignTemplateId, isOpen } = confirmModal

    const onComplete = () => {
      onGetCampaignTemplates(filter)
    }

    setConfirmModal({ isOpen: !isOpen, campaignTemplateId: null })
    onDeleteCampaignTemplate({ _id: campaignTemplateId }, onComplete)
  }
  ...
  const handleCampaignTemplateStatusChange = status => {
    const callback = () => {
      setSelected([])
      onGetCampaignTemplates(filter)
    }

    onPutCampaignTemplatesStatus({ _id: selected, status }, callback)
  }
      ...
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs
            ...
            breadcrumbItem={props.t("campaign_templates")}
            breadcrumbItems={[{ title: props.t("campaign_templates") }]}
          />
                                ...
                                <div className="text-sm-left text-md-right">
                                  {accesses.canAdd && (
                                    <Button
                                      ...
                                      onClick={() =>
                                        history.push("/campaign-templates/add")
                                      }
                                    >
                                      <i className="mdi mdi-plus mr-1" />
                                      {props.t("add_campaign_template")}
                                    </Button>
                                  )}
                                </div>
                              </Col>
                                    ...
                                    <div className="filter form-group d-flex align-items-center mb-0 mr-3">
                                      ...
                                      <div className="w-auto pl-3">
                                        <select
                                          className="custom-select w-100"
                                          onChange={e =>
                                            handleCampaignTemplateStatusChange(
                                              e.target.value
                                            )
                                          }
                                          style={{ minWidth: "106px" }}
...
CampaignTemplate.propTypes = {
  ...
  totalCampaignTemplate: PropTypes.number,
  onGetCampaignTemplates: PropTypes.func,
  onDeleteCampaignTemplate: PropTypes.func,
  onPutCampaignTemplatesStatus: PropTypes.func,
}

const mapStateToProps = ({ Settings, CampaignTemplate }) => ({
  settings: Settings.settings,
  loading: CampaignTemplate.loading,
  campaignTemplates: CampaignTemplate.campaignTemplates,
  totalCampaignTemplate: CampaignTemplate.totalCampaignTemplate,
})

const mapDispatchToProps = dispatch => ({
  onGetCampaignTemplates: data => dispatch(getCampaignTemplates(data)),
  onDeleteCampaignTemplate: (data, callback) =>
    dispatch(deleteCampaignTemplate(data, callback)),
  onPutCampaignTemplatesStatus: (data, callback) =>
    dispatch(putCampaignTemplatesStatus(data, callback)),
})
...
export default withRouter(
  connect(
    mapStateToProps,
    mapDispatchToProps
  )(withTranslation()(CampaignTemplate))
)
