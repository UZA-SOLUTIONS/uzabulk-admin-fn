// @flow
import { all, call, fork, takeEvery, put, delay } from "redux-saga/effects"
...
function* changeLeftSidebarType({ payload: { sidebarType, isMobile } }) {
  try {
    console.log("sidebarType", sidebarType, isMobile)
    switch (sidebarType) {
      case "compact":
      ...
      case "condensed":
        ...
        if (isMobile) return yield call(manageBodyClass, "vertical-collpsed", "add")
        // yield call(manageBodyClass, "vertical-collpsed", "add")
        break
      default:
...
function* toggleLeftSidebarType({ payload: { sidebarType, isMobile } }) {
  try {
    yield delay(80)
    yield put(changeSidebarTypeAction("default", isMobile))
  } catch (error) { }
}
...
export function* watchToggleLeftSidebarType() {
  yield takeEvery(TOGGLE_SIDEBAR_TYPE, toggleLeftSidebarType)
}
...
function* LayoutSaga() {
  yield all([
    ...
    fork(watchToggleLeftSidebarType),
